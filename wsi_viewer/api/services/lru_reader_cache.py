...
from .wsi_reader import open_slide
import logging
...
class LRUReaderCache:
    ...
    def add(self, file_path):
        ...
        new_reader = open_slide(file_path)

        # 缓存已满时淘汰最久未使用的对象
        if len(self.cache) >= self.max_size:
            lru_key, _ = self.cache.popitem(last=False)
            logger.info(f"[ LRUReaderCache ] LRU淘汰: {lru_key}")
...
if __name__ == "__main__":
    ...
    for idx, file in enumerate(files, 1):
        ...
        if existing_reader:
            print(f"直接使用已存在的Reader: {existing_reader.slide_path}")
            print(existing_reader.read())
            continue

        # 新增Reader对象
        new_reader = cache.add(file)
        print(f"创建新Reader: {new_reader.slide_path}")
